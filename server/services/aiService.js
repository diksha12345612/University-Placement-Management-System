...
/**
 * Get previously generated questions for a specific student's topic
 * Only returns questions generated by that specific student
 */
const getPreviousQuestionsByUser = async (topic, userId) => {
    try {
        ...
        console.log('[AI] Warning: Could not fetch user question history:', err.message);
        return [];
    }
};
 ...
 * Now tracks which user generated the questions
 */
const saveGeneratedQuestions = async (topic, difficulty, questions, mockTestId, userId) => {
    try {
        ...
        console.log(`[AI] ✅ Saved ${questions.length} questions to QuestionBank for ${topic} (user: ${userId})`);
    } catch (err) {
 ...
 * @param {string} userId - Student ID for isolation (optional)
 */
const generateMockTest = async (topic, difficulty = 'Medium', count = 5, questionTypes = 'mix', userId = null) => {
    const isTechnical = !['aptitude', 'soft skills', 'english', 'verbal', 'logical reasoning', 'quantitative'].some(t =>
        topic.toLowerCase().includes(t)
    );

    console.log(`[AI] Generating ${isTechnical ? 'Technical' : 'Non-Technical'} test for topic: "${topic}", student: ${userId || 'unknown'}`);

    // Get ONLY this student's previously generated questions to avoid duplicates
    let previousQuestions = [];
    if (userId) {
        previousQuestions = await getPreviousQuestionsByUser(topic, userId);
    }
    const previousQuestionTexts = previousQuestions.map(q => q.question);
    const deduplicationNote = previousQuestionTexts.length > 0 
        ? `\n\nIMPORTANT - AVOID YOUR PREVIOUSLY GENERATED QUESTIONS:\n${previousQuestionTexts.map((q, i) => `${i+1}. ${q.substring(0, 80)}...`).join('\n')}\n\nGenerate COMPLETELY NEW and DIFFERENT questions that have never been used before in YOUR previous tests.`
        : '';
